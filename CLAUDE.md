...
- **Doc build: every minted block fails with "minted v3+ executable is not installed"** and the log shows `TypeError: ArgParser.__init__() got an unexpected keyword argument 'color'`: TeX Live 2025 ships `latexminted` 0.6.0, which breaks under Python 3.14. Fix without touching TeX Live (see issue #516): `pipx install latexminted` (0.7.1+ lands in `~/.local/bin`, ahead of TeX Live on `PATH`, which `minted.sty` honours). latexminted 0.7 then demands `minted.sty` >= 3.8.0, which in turn wants current `fvextra` and `latex2pydata`, so install those three from CTAN into `TEXMFHOME` (`kpsewhich -var-value TEXMFHOME`, usually `~/texmf`): unzip each of `macros/latex/contrib/{minted,fvextra,latex2pydata}.zip`, run `tex <name>.ins` where no `.sty` ships, and copy the `.sty` files to `~/texmf/tex/latex/<name>/`. Verify with `kpsewhich minted.sty` and a two-pass `xelatex -shell-escape` of a one-block test document.
- **Doc build: minted blocks render as a literal `<MINTED>` placeholder** and the log has a `latexminted` `FileNotFoundError` for `.../_minted/<hash>.highlight.minted`: a stray `_minted` cache directory somewhere under `TEXMFHOME` (e.g. left by building a package's own docs inside `~/texmf/tex/latex/<pkg>/`) is found by kpathsea for the relative `_minted` path, so the real cache directory is never created. `kpsewhich _minted/default.style.minted` names the culprit; move it out of `TEXMFHOME`.
